...
#Do not call twice it will cause problem, in general do not call interactive code twice 
...
myNumericVector10 <- myNumericVector + 10
...
#factor reflecting groups in data, can be useful in research 
# matrix
my.matrix <- as.matrix(read_delim("matrix.txt")) # notice how we can nest functions
#tinyverse only deals with dataframe, so we need a nested function
...
myVectorSubset <- my_character_vector[2:3]
myMatrixSubset <- my.matrix[1,1:3]
